...
class GridShape(object):
    ...
    def triangulate_square_vrml(self, square, invert_normals=False):
        ''' assume points like this for right hand rule
            0  1
            2  3
        '''
        if invert_normals:
            return [square[0], square[1], square[3], -1,
                    square[0], square[3], square[2], -1]
        else:
            return [square[0], square[3], square[1], -1,
                    square[0], square[2], square[3], -1]
...
class MeshSandwich(GridShape):
    ...
    def triangulate(self):
        ...
        triangles.extend(
            self.bottom.triangulate(invert_normals=self.invert_normals))
...
class Mesh(GridShape):
    ...
    def get_irregular_grid_vrml(self):
        coords = []
        faces = []
    ...
    def create_ceiling(self, nz_mm, dezfactor):
        ...
        # window on mesh data to be decimated, doesn't include the data "in the
        # walls"
        ...
        full_ceiling_mesh = np.ndarray(
            (d_mesh_size[0] + 2, d_mesh_size[1] + 2, 3))
        cell_mesh = full_ceiling_mesh[1:-1, 1:-1]

        # filter view of the output, address values to be filled by scanning an
        # area
        ...
        if (True):
            for x in range(0, cell_mesh.shape[1]):
                ...
                # the line includes all the pixels along y=0 and x from the
                # begning to the end of a cell
                line = src[0,
        ...
        if (True):
            for y in range(0, cell_mesh.shape[0]):
                ...
                line = src[y * pix_per_cell[0] + acc(y, fstep[0]) + (1 if y == 0 else 0):
                           (y + 1) * pix_per_cell[0] + acc(y, fstep[0]) -
                           1 - (1 if y == (cell_mesh_shape[0] - 1) else 0),
                           0]
                elevations = line[:, 2]
                line_min = elevations.argmin()
                # x+1 to skip the cornder
    ...
    def transform_border(self, scale, translate, depth=1):
        # x
        self.mesh[0:depth, :] = np.multiply(
            self.mesh[0:depth, :], np.array(scale))
        self.mesh[0:depth, :] = np.add(
            self.mesh[0:depth, :], np.array(translate))
        self.mesh[-depth:,
                  :] = np.multiply(self.mesh[-depth:, :], np.array(scale))
        self.mesh[-depth:,
                  :] = np.add(self.mesh[-depth:, :], np.array(translate))

        # y
        self.mesh[
            depth:-depth, 0:depth] = np.multiply(self.mesh[depth:-depth, 0:depth], np.array(scale))
        self.mesh[
            depth:-depth, 0:depth] = np.add(self.mesh[depth:-depth, 0:depth], np.array(translate))
        self.mesh[depth:-depth, -depth:
                  ] = np.multiply(self.mesh[depth:-depth, -depth:], np.array(scale))
        self.mesh[depth:-depth, -depth:
                  ] = np.add(self.mesh[depth:-depth, -depth:], np.array(translate))
    ...
    def transform_border_stepwise(self, scale_start, scale_step, translate_start, translate_step, depth=1):
        ...
        def transform_edge(ystart, ystop, xstart, xstop):
            ...
            self.mesh[yslice, xslice] = np.multiply(
                self.mesh[yslice, xslice], np.add(scale_start, d * scale_step))
            self.mesh[yslice, xslice] = np.add(
                self.mesh[yslice, xslice], np.add(translate_start, d * translate_step))
